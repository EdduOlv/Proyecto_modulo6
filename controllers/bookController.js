...
exports.getAllBooks = async (req, res) => {
    try {
        const books = await Book.find({}).select("-summary");;
        res.json({books}) 
    } catch (error) {
        res.status(500).json({
...
exports.updateBook = async (req, res) => {
    const {id, title, author, publisher, summary, pageCount, price} = req.body;
    try {
        const updateBook = 
	        await Book.findByIdAndUpdate(id, {title, author, publisher, summary, pageCount, price}, { new: true })
        res.json(updateBook)
    } catch (error) {       
        res.status(500).json({
